...
contract HKS is Ownable {
    ...
    event onWithdraw(address indexed customerAddress, uint256 hexWithdrawn);
    ...
    function roll() public onlyDivis {
    ...
    function exit() external {
        address _customerAddress = msg.sender;
        uint256 _tokens = SafeMath.sub(
            tokenBalanceLedger_[_customerAddress],
            lockedTokenBalanceLedger[_customerAddress]
        );
    ...
    function sell(uint256 _amountOfTokens) public onlyTokenHolders {
        ...
        uint256 _maintenance = SafeMath.div(
            SafeMath.mul(_undividedDividends, maintenanceFee_),
            100
        );
        erc20.transfer(maintenanceAddress, _maintenance);

        uint256 _tewkenaire = SafeMath.div(
            SafeMath.mul(_undividedDividends, tewkenaireFee_),
            100
        );
        totalFundCollected += _tewkenaire;
        erc20.transfer(distributionAddress, _tewkenaire);

        uint256 _dividends = SafeMath.sub(
            _undividedDividends,
            SafeMath.add(_maintenance, _tewkenaire)
        );
        ...
        int256 _updatedPayouts = (int256)(
            profitPerShare_ * _amountOfTokens + (_taxedHEX * magnitude)
        );
    ...
    function approveAddress1(address _proposedAddress) public onlyOwner {
        approvedAddress1 = _proposedAddress;
    }

    function approveAddress2(address _proposedAddress) public onlyCustodian {
    ...
    function balanceOf(address _customerAddress, bool stakable)
        public
        view
        returns (uint256)
    {
        ...
        returns (uint256)
    {
        uint256 _dividends = SafeMath.div(
            SafeMath.mul(_hexToSpend, entryFee_),
            100
        );
        ...
        returns (uint256)
    {
        ...
        uint256 _undividedDividends = SafeMath.div(
            SafeMath.mul(_incomingHEX, entryFee_),
            100
        );
        ...
        uint256 _dividends = SafeMath.sub(
            _undividedDividends,
            SafeMath.add(_tewkenaire, _maintenance)
        );
        ...
        isStakeActivated
    {
        ...
        lockedTokenBalanceLedger[msg.sender] = SafeMath.add(
            lockedTokenBalanceLedger[msg.sender],
            _amount
        );
    ...
    function stakeEnd(
        ...
        lockedTokenBalanceLedger[msg.sender] = SafeMath.sub(
            lockedTokenBalanceLedger[msg.sender],
            _stakedAmount
        );
