...
contract TOKEN {
    ...
    function stakeLists(address owner, uint256 stakeIndex)
        external
        view
...
contract HRS is Ownable {
    ...
    uint256 ACTIVATION_TIME = 1582416000;
    ...
    event onDistribute(address indexed customerAddress, uint256 tokens);
    ...
    event onTokenAppreciation(uint256 tokenPrice, uint256 timestamp);
    ...
    uint256 internal constant magnitude = 1e8;

    uint8 internal constant transferFee = 1;
    uint8 internal constant buyInFee = 4;
    uint8 internal constant sellOutFee = 4;
    uint8 internal constant devFee = 1;
    uint8 internal constant hexTewFee = 1;
    ...
    function buyFor(uint256 _amount, address _customerAddress)
        public
        returns (uint256)
    {
        checkAndTransferHEX(_amount);
    ...
    function _purchaseTokens(address _customerAddress, uint256 _incomingHEX)
        private
        returns (uint256)
    {
        ...
        require(
            _amountOfTokens > 0 &&
                _amountOfTokens.add(tokenSupply) > tokenSupply
        );

        tokenBalanceLedger[_customerAddress] = tokenBalanceLedger[_customerAddress]
            .add(_amountOfTokens);
    ...
    function purchaseTokens(address _customerAddress, uint256 _incomingHEX)
        private
        isActivated
        returns (uint256)
    {
        ...
        uint256 _amountOfTokens = _purchaseTokens(
            _customerAddress,
            _incomingHEX.sub(_fee).sub(_devFee).sub(_hexTewFee)
        );
    ...
    function sell(uint256 _amountOfTokens) public isActivated onlyTokenHolders {
        ...
        uint256 _lockedToken = (
            lockedHexBalanceLedger[_customerAddress].mul(magnitude)
        ) / tokenPrice;

        require(
            _amountOfTokens > 0 &&
                _amountOfTokens <=
                tokenBalanceLedger[_customerAddress].sub(_lockedToken)
        );
        ...
        returns (bool)
    {
        ...
        tokenBalanceLedger[_customerAddress] = tokenBalanceLedger[_customerAddress]
            .sub(_amountOfTokens);
        tokenBalanceLedger[_toAddress] = tokenBalanceLedger[_toAddress].add(
            _taxedTokens
        );
    ...
    function stakeStart(uint256 _amount, uint256 _days)
        public
        isStakeActivated
    {
        ...
        (_stakeID, , _stakeShares, _lockedDay, _stakedDays, , ) = erc20
            .stakeLists(address(this), _stakeIndex);

        uint256 _uniqueID = uint256(
            keccak256(abi.encodePacked(_stakeID, _stakeShares))
        ); // unique enough
        ...
        stakeLists[msg.sender][_uniqueID] = StakeStore(
            _stakeID,
            _amount,
            _stakeShares,
            _lockedDay,
        ...
        lockedHexBalanceLedger[msg.sender] = SafeMath.add(
            lockedHexBalanceLedger[msg.sender],
            _amount
        );
        ...
        emit onStakeStart(
            msg.sender,
            _uniqueID,
            calculateTokensReceived(_amount, false),
            now
        );
    }
    ...
    function _stakeEnd(
        ...
        ) = erc20.stakeLists(address(this), _stakeIndex);
        require(
            stakeLists[msg.sender][_uniqueID].started == true &&
                stakeLists[msg.sender][_uniqueID].ended == false
        );
        require(
            stakeLists[msg.sender][_uniqueID].stakeID == _stakeIdParam &&
                _stakeIdParam == _stakeID
        );
        require(
            stakeLists[msg.sender][_uniqueID].hexAmount ==
    ...
    function stakeEnd(
        ...
        if (_unlockedDay == 0) {
            stakeLists[msg.sender][_uniqueID].unlockedDay = uint16(
                erc20.currentDay()
            ); // no penalty/penalty/reward
        } else {
        ...
        if (_amount <= _stakedAmount) {
            ...
            tokenBalanceLedger[msg.sender] = SafeMath.sub(
                tokenBalanceLedger[msg.sender],
                _difference
            );
        ...
        lockedHexBalanceLedger[msg.sender] = SafeMath.sub(
            lockedHexBalanceLedger[msg.sender],
            _stakedAmount
        );
    ...
    function setHexStaking(bool _stakeActivated) public onlyOwner {
    ...
    function balanceOf(address _customerAddress, bool _stakeable)
        public
        view
        returns (uint256)
    {
    ...
    function calculateTokensReceived(uint256 _hexToSpend, bool _includeFees)
        public
        view
        returns (uint256)
    {
    ...
    function hexBalanceOf(address _customerAddress, bool _stakeable)
        public
        view
        returns (uint256)
    {
    ...
    function hexBalanceOfNoFee(address _customerAddress, bool _stakeable)
        public
        view
        returns (uint256)
    {
